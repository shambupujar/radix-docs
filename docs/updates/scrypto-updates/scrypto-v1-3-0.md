...
The `TransactionBuilder::new_v2()` can also be used to build a `PreviewTransactionV2`, with the `build_preview_transaction(transaction_intent_signer_public_keys)` method, which can be used before signing/notarizing. The `PreviewTransactionV2` can be used in the new preview-v2 API on the [Core API](/api-reference/core-api-specs.html#tag/Transaction/paths/~1transaction~1preview-v2/post) or [Gateway API](/api-reference/gateway-api-specs.html#operation/TransactionPreviewV2).
