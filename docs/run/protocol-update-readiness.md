...
For some examples of protocol updates:

- The original protocol version was called `babylon`, and introduced with the `v1.0.0` node and immediately enacted.
- The first protocol update is called `anemone`, introduced with the [v1.1.0](https://github.com/radixdlt/babylon-node/releases/tag/v1.1.0) node. It will enact [on mainnet in accordance with this configuration](https://github.com/radixdlt/babylon-node/blob/release/anemone/core-rust/state-manager/src/protocol/protocol_configs/mainnet_protocol_config.rs), when sufficient validators have signalled readiness for the update, by using the readiness signal `220e2a4a4e86e3e6000000000anemone`. This won’t be enacted until the start of epoch 70019 at the earliest, estimated to be approximately `2024-02-05T18:00:57.229Z`. It will enact at the start of an epoch. Its enactment requires validators representing at least 75% of the total delegated stake to have signalled readiness, and for this threshold to have been met for at least the 4 preceding days.
